...
class Sentence():
    ...
    def mark_mine(self, cell):
        ...
        """
        # Only update if cell is in the sentence
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 3. Get neighbors that are not already known to be safe or mines
        neighborCells = set()
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i, j) == cell:
        ...
        # 4. Add new sentence to knowledge base
        new_sentence = Sentence(neighborCells, count)
        if new_sentence not in self.knowledge:
            self.knowledge.append(new_sentence)

        # 5. Repeat inference until no new info can be added
        changed = True
        while changed:
            changed = False
    ...
    def make_safe_move(self):
        ...
        for move_safe in self.safes:
            if move_safe not in self.moves_made and move_safe not in self.mines:
                return move_safe
        return None
